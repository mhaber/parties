...
### Run Wordfish
wf.res <- wordfish(wfm, dir=c(1582, 1609)) #dir=c() is used to anchor documents; i.e. one text has a lower value than another
...
# party leader vector
leader <- rep(0,1659) 
leader[c(24,54,84,93,193,149,300,338,389,493,557,599,674,713,761,820,854,903,923,999,
         948,1013,1058,1060,1068,1106,1110,1167,1175, 1230,1242,1290,1293,1304,1317,
         1371,1383,1433,1479,1498,1526,1583,1610)] <- 1

# party vector
party <- rep("CDU",1659)
party[c(54,93,149,338,557,713,820,923,948,1013,1068,1110,1175,1242,1293,1317,
        1383,1479,1526,1610)] <- "CSU"
...
# clean speaker names
thetas$speaker <- gsub("^.*_(.*).txt.*$","\\1",thetas$speaker) 
thetas$speaker <- gsub("^.*_(.*).txt.*$","\\1",thetas$speaker)
...
p1 <- ggplot(subset(thetas,leader==1), aes(x=congress, y=mean, group=1)) +
  ...
  #geom_point(aes(y=partyMean, shape=20, size=4))  +
